...
d = pandas.read_csv('popRef.csv' ,  on_bad_lines='skip' , delimiter=';')
d = d.values.tolist()
...
def second(listGeometry, pop , bruiteur , echelle , spliteur ):
    ...
    bruiteur = bruiteur 
    echelle = bruiteur + echelle
    spliteur = echelle + spliteur
...
def calcul(listeA , listeB) :
    ...
    for i in range(len(listeA)):
        ...
        if i < 3 : 
            ...
            sum_1 += abs( listeA[i] - listeB[i]) 
            
        elif i < len(listeA)-3 and i >= 3 : 
        
            sum_2 += abs( listeA[i] - listeB[i]) 
            
        else : 
        
            sum_3 += abs( listeA[i] - listeB[i]) 
        
    return ( sum_1 , sum_2 , sum_3 , sum_ ) 
...
( sum_1 , sum_2 , sum_3 , sum_ )  = calcul(density , listeB)
